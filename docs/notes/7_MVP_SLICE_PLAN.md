...
| --- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| 1   | Auth end-to-end                                  | REST signup/login/verify/resend, custom email/password auth, `/verify-email` page, console email transport, CORS                                                                                                                                | ✅ Done — backend verified by `scripts/test-slice-1.sh`; browser flow live           |
| 1.1 | Slice 1 polish                                   | Signup vs sign-in visual differentiation + expanded signup fields + VerifyEmail double-fire bug                                                                                                                                                 | Browser-tested signup with full profile fields; verify link reliably reports success |
...
| 3   | Create/Edit Community + Event forms              | New form pages (`/communities/new`, `/communities/:id/edit`, `/communities/:id/events/new`); wire to existing GraphQL mutations; auto-assign steward role on community creation; event popup edit; recurring event toggle (stubbed — not wired) | User creates a community + event from the UI; steward can edit both                  |
| 4   | Personal Calendar + Event Detail Popup with .ics | `/calendar` route, event popup, install `ical-generator`, server `.ics` endpoint, self-email action; wire recurring event fields to mutation (deferred from Slice 3)                                                                            | Calendar shows subscribed events; tile → popup → download/email `.ics`               |
