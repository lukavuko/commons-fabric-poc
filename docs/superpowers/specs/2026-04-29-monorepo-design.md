...
The root-level `prisma/` folder and `prisma.config.ts` move into `db`. The root-level `server/` and `src/` directories move into `apps/server/` and `apps/web/` respectively.
...
**`migrator`**: built from `db`; runs `npx prisma migrate deploy` then exits. All app services declare `depends_on: migrator: condition: service_completed_successfully`.
...
```
Browser   → apps/auth   → db  (Session, User)
Browser   → apps/server → db  (all other models)

apps/scheduler   → db  (INSERT job_queue rows)
apps/executioner → db  (SELECT/UPDATE job_queue rows)
