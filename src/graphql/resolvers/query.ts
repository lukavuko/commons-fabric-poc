...
export const Query = {
  ...
  communities: async (_: unknown, args: { filter?: Record<string, unknown>; pagination?: { limit?: number; offset?: number } }, ctx: Context) => {
    const { filter, pagination } = args;
    return ctx.prisma.community.findMany({
      where: {
        ...(filter?.city ? { city: { contains: filter.city as string, mode: 'insensitive' } } : {}),
        ...(filter?.province ? { province: { contains: filter.province as string, mode: 'insensitive' } } : {}),
  ...
  searchCommunities: (_: unknown, args: { query: string; limit?: number }, ctx: Context) =>
  ...
  events: async (_: unknown, args: { communityId?: string; fromDate?: string; toDate?: string }, ctx: Context) =>
  ...
  comments: (_: unknown, args: { parentEntityType: string; parentEntityId: string }, ctx: Context) =>
